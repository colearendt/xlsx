...
public class RCellBlock {
		...
		for (int i = 0; i < nRows; i++) {
			...
			for (int j = 0; j < nCols; j++) {
				cells[j][i] = create ? r.createCell(startColIndex+j)
                        : r.getCell(startColIndex+j, Row.MissingCellPolicy.CREATE_NULL_AS_BLANK);
			}
		}
	}
    ...
    public void setColData( int colIndex, int rowOffset, double[] data, boolean showNA, CellStyle style ){
        ...
        for (int i=0; i<data.length; i++) {
            if ( showNA || !RInterface.isNA(data[i])) {
                ...
                colCells[rowOffset+i].setCellType(CellType.BLANK);
            }
    ...
    public void setRowData( int rowIndex, int colOffset, String[] data, boolean showNA, CellStyle style ){
        for (int i=0; i<data.length; i++) {
            if ( showNA || !RInterface.isNA(data[i])) {
                ...
                cells[colOffset+i][rowIndex].setCellType(CellType.BLANK);
            }
		...
		for (int j = startColumn; j <= endColumn; j++) {
			for (int i = startRow; i <= endRow; i++) {
				if (showNA || !RInterface.isNA(data[i])) {
					...
					cells[j][i].setCellType(CellType.BLANK);
				}
				if (newStyle != null) {
     ...
     */
    public void setCellStyle( CellStyle style, int rowIx, int colIx )
    {
    ...
    public void setFill( final short foreground, final short background, 
    	final FillPatternType pattern, int[] rowIndices, int[] colIndices )
    {
     ...
     * The other properties of cell styles are preserved.
     */
    public void setFill( final XSSFColor foreground, final XSSFColor background, final FillPatternType pattern,
     ...
     * If provided border type is NONE the corresponding border is not modified.
     * The other properties of cell styles are preserved.
     */
    public void putBorder( final BorderStyle borderTop, final short topBorderColor,
                           final BorderStyle borderBottom, final short bottomBorderColor,
                           final BorderStyle borderLeft, final short leftBorderColor,
                           final BorderStyle borderRight, final short rightBorderColor,
                           int[] rowIndices, int[] colIndices )
    {
        modifyCellStyle(rowIndices, colIndices, new CellStyleModifier() {
            public void modify( CellStyle style ) {
     ...
     * The other properties of cell styles are preserved.
     */
    public void putBorder( final BorderStyle borderTop, final XSSFColor topBorderColor,
                           final BorderStyle borderBottom, final XSSFColor bottomBorderColor,
                           final BorderStyle borderLeft, final XSSFColor leftBorderColor,
                           final BorderStyle borderRight, final XSSFColor rightBorderColor,
                           int[] rowIndices, int[] colIndices )
    {
        modifyCellStyle(rowIndices, colIndices, new CellStyleModifier() {
            public void modify( CellStyle style ) {
