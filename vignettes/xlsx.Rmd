...
there are already several solutions, `RODBC`\cite{RODBC},
`xlsReadWrite`\cite{xlsReadWrite},
`RExcelInstaller`\cite{RExcelInstaller}, etc.  While some of
...
as shown by the package `RExcelXML`\cite{RExcelXML}.  All the
...
the focus of the `xlsx` package has been only on Excel files, if
