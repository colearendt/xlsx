...
## The dataset
numbercol <- 10
mydata <- as.data.frame(lapply(1:numbercol,function(x){runif(30, 0,200)}))
...
## the xlsx jars are not available... need to fix
addDataFrame(mydata, sh
             , col.names=TRUE
             , row.names = FALSE
             , startRow = workrow
             , startColumn = 1 
             , colStyle = NULL #setNames(cslist,1:10)
             , colnamesStyle = NULL ## CellStyle(wb) + f$subtitle + alignCenter + fillPrimary
             )
...
addDataFrame(mydata[1,], sh
             ...
             , startRow = workrow
             , startColumn = 1
             , colStyle = setNames(lapply(cslist,function(x){x + fillSecondary}),1:10)
             )
```